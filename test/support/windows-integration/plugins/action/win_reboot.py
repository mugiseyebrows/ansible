...
class ActionModule(RebootActionModule, ActionBase):
    ...
    async def run_test_command(self, distribution, **kwargs):
        ...
        await super(ActionModule, self).run_test_command(distribution, **kwargs)
    ...
    async def perform_reboot(self, task_vars, distribution):
        ...
        if reboot_result['rc'] == 1190 or (reboot_result['rc'] != 0 and "(1190)" in reboot_result['stderr']):
            ...
            result1 = await self._low_level_execute_command(self._connection._shell._encode_script('shutdown /a'),
                                                      sudoable=self.DEFAULT_SUDOABLE)

            # Initiate reboot again
            result2 = await self._low_level_execute_command(reboot_command, sudoable=self.DEFAULT_SUDOABLE)
